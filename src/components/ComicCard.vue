...
const charactersList = computed(() =>
  props.comic.characters.item.maps((c) => c.name)
);

const creatorsList = computed(() =>
  props.comic.creators.item.maps((c) => c.name)
);
...
<template>
  <CardView :data-testid="comic.id">
    ...
    <template v-slot:default>
      <img class="aspect-[150/228] shadow-xl float-left mr-4" :src="`${comic.thumbnail.path}.${comic.thumbnail.extension}`" witdh="150"/>
      <dl>
        <dt>Series:</dt>
